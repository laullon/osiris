...
impl Renderer {
    ...
    pub fn paint(
        ...
        if let Some(mut pixmap) = PixmapMut::from_bytes(raw_bytes, buf_width, buf_height) {
            ...
            self.tui.draw_box(&mut pixmap, &metrics, 1, 1, metrics.cols - 2, GRID_ROWS - 3, cyan);
            
            let title = " OSIRIS MISSION TERMINAL ";
            self.tui.draw_string(&mut pixmap, &metrics, title, (metrics.cols / 2).saturating_sub(title.len() / 2), 1, cyan);
