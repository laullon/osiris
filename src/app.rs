...
impl ApplicationHandler for OsirisApp {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        if self.window.is_none() {
            let window = Rc::new(event_loop.create_window(Window::default_attributes().with_title("OSIRIS").with_fullscreen(Some(winit::window::Fullscreen::Borderless(None)))).unwrap());
    ...
    fn window_event(&mut self, _el: &ActiveEventLoop, _id: WindowId, event: WindowEvent) {
        match event {
            ...
            WindowEvent::KeyboardInput { event: KeyEvent { state, physical_key: PhysicalKey::Code(key), repeat: false, .. }, .. } => {
                if let Some(cmd) = self.map_key(key) {
                    if state == ElementState::Pressed { self.press_command(cmd); } 
    ...
    fn about_to_wait(&mut self, _el: &ActiveEventLoop) {
        while let Some(ev) = self.gil.next_event() {
            match ev.event {
                EventType::ButtonPressed(btn, _) => if let Some(cmd) = self.map_btn(btn) { self.press_command(cmd); }
                EventType::ButtonReleased(btn, _) => if let Some(cmd) = self.map_btn(btn) { self.release_command(cmd); }
                _ => {}
            }
        }
        ...
        for (cmd, state) in self.active_commands.iter_mut() {
            if !matches!(cmd, NavCommand::Up | NavCommand::Down) { continue; }
...
impl OsirisApp {
    ...
    fn press_command(&mut self, cmd: NavCommand) {
        if !self.active_commands.contains_key(&cmd) {
            let now = Instant::now();
            self.active_commands.insert(cmd, CommandState { last_trigger: now, started_at: now, repeating: false });
            self.game_list.handle_command(cmd);
            self.last_cmd = Some((cmd, now));
            if let Some(win) = &self.window { win.request_redraw(); }
        }
    }
    fn release_command(&mut self, cmd: NavCommand) { self.active_commands.remove(&cmd); }
    fn map_key(&self, key: KeyCode) -> Option<NavCommand> {
        match key { KeyCode::ArrowUp => Some(NavCommand::Up), KeyCode::ArrowDown => Some(NavCommand::Down), KeyCode::Space => Some(NavCommand::Select), KeyCode::Escape => Some(NavCommand::Back), _ => None }
    }
    fn map_btn(&self, btn: Button) -> Option<NavCommand> {
        match btn { Button::DPadUp => Some(NavCommand::Up), Button::DPadDown => Some(NavCommand::Down), Button::South => Some(NavCommand::Select), Button::East => Some(NavCommand::Back), _ => None }
    }
}